import argparse
import time
import torch
from torch.optim import SGD, Adam, lr_scheduler

from tqdm.auto import tqdm

from datasets import get_datasets, get_dataloaders
from model import build_model
from utils import save_model, save_plots

STEP_SIZE = 7
MOMENTUM = 0.9
GAMMA = 0.1


def train(model, trainloader, optimizer, scheduler, loss_criteria):
    model.train()
    print('Training')
    train_running_loss = 0.0
    train_running_accuracy = 0
    counter = 0
    for i, data in tqdm(enumerate(trainloader), total=len(trainloader)):
        counter += 1
        image, labels = data
        image = image.to(device)
        labels = labels.to(device)
        optimizer.zero_grad()
        # forward pass
        outputs = model(image)
        # calculate loss
        loss = loss_criteria(outputs, labels)
        train_running_loss += loss.item()
        # calculate accuracy
        _, preds = torch.max(outputs.data, 1)
        train_running_accuracy += (preds == labels).sum().item()
        # backpropagate loss
        loss.backward()
        # Update weights
        optimizer.step()

    #scheduler.step()

    # Epoch Loss and accuracy
    epoch_loss = train_running_loss/counter
    epoch_acc = 100. * (train_running_accuracy/len(trainloader.dataset))
    return epoch_loss, epoch_acc


def validate(model, validloader, loss_criteria):
    model.eval()
    print('Validation')
    valid_running_loss = 0.0
    valid_running_accuracy = 0
    counter = 0
    with torch.no_grad():
        for i, data in tqdm(enumerate(validloader), total=len(validloader.dataset)):
            counter += 1
            image, labels = data
            image = image.to(device)
            labels = labels.to(device)
            # forward pass
            outputs = model(image)
            # calculate loss
            loss = loss_criteria(outputs, labels)
            valid_running_loss += loss.item()
            # calculate accuracy
            _, preds = torch.max(outputs.data, axis=1)
            valid_running_accuracy += (preds == labels).sum().item()

    # Epoch Loss and accuracy
    epoch_loss = valid_running_loss / counter
    epoch_acc = 100. * (valid_running_accuracy / len(validloader.dataset))
    return epoch_loss, epoch_acc


if __name__ == "__main__":
    # construct argument parser
    parser = argparse.ArgumentParser()
    parser.add_argument('-e', '--epochs', type=int, default=20, help='Number of epochs to train the model')
    parser.add_argument('-pt', '--pretrained', action='store_true', help='Whether to use pretrained weights')
    parser.add_argument('-lr', '--learning_rate', type=float, default=0.0001,
                        help='Learning rate for training the model')
    args = vars(parser.parse_args())
    # load training and validation datasets
    dataset_train, dataset_valid, dataset_classes = get_datasets(args['pretrained'])
    print(f"[INFO]: Number of training images: {len(dataset_train)}")
    print(f"[INFO]: Number of validation images: {len(dataset_valid)}")
    print(f"[INFO]: Class names: {dataset_classes}\n")
    # Load the training and validation data loaders.
    train_loader, valid_loader = get_dataloaders(dataset_train, dataset_valid)
    print(f"[INFO]: train loader size: {len(train_loader)}")
    print(f"[INFO]: valid loader size: {len(valid_loader)}")

    # Learning parameters
    lr = args['learning_rate']
    epochs = args['epochs']
    device = ('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Computation device: {device}")
    print(f"Learning rate: {lr}")
    print(f"Number of epochs the model is trained for: {epochs}\n")

    classification_model = build_model(pretrained=args['pretrained'],
                                       finetune=True,
                                       num_classes=len(dataset_classes)).to(device)

    loss_criterion = torch.nn.CrossEntropyLoss()
    optimizer = Adam(classification_model.parameters(), lr=lr)
    # decay lr by a factor of 0.1 after every 7 epochs
    lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=STEP_SIZE, gamma=GAMMA)

    train_loss, valid_loss = [], []
    train_acc, valid_acc = [], []
    for epoch in range(epochs):
        st = time.time()
        print(f"INFO: Epoch {epoch+1} of {epochs}")
        train_epoch_loss, train_epoch_acc = train(classification_model, train_loader,
                                                  optimizer, lr_scheduler, loss_criterion)
        valid_epoch_loss, valid_epoch_acc = validate(classification_model, valid_loader, loss_criterion)
        train_loss.append(train_epoch_loss)
        train_acc.append(train_epoch_acc)
        valid_loss.append(valid_epoch_loss)
        valid_acc.append(valid_epoch_acc)
        print(f"Training loss: {train_epoch_loss:.3f}, training acc: {train_epoch_acc:.3f}")
        print(f"Validation loss: {valid_epoch_loss:.3f}, validation acc: {valid_epoch_acc:.3f}")

    total_time = time.time() - st
    # Save trained model weights
    save_model(epochs, classification_model, optimizer, loss_criterion, args['pretrained'])
    # save training loss and accuracy plots
    save_plots(train_acc, valid_acc, train_loss, valid_loss, args['pretrained'])
    print('Training complete in {:.0f}m {:.0f}s'.format(total_time // 60, total_time % 60))
